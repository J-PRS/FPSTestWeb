...
const logger = new ChildLogger('UWSAdapter');

/**
 * uWS (µWebSockets) -based networking adapter
 * Implements INetworkAdapter for connecting to uWS servers
 * 
 * Note: uWS is a server-side library. This adapter uses the browser's
 * native WebSocket API to connect to uWS servers, with uWS-specific
 * optimizations and protocol handling.
 */
export class UWSAdapter implements INetworkAdapter {
  ...
  private async doConnect(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onopen = () => {
          logger.info('Connected to uWS server');
        ...
        this.ws.onclose = () => {
          logger.info('Disconnected from uWS server');
