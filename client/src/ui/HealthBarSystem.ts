...
export class HealthBarSystem {
  ...
  spawn(playerId: string, damage: number, health: number): void {
    let bar = this.bars.get(playerId);
    if (bar) {
      if (bar.delayTimer > 0) {
        bar.damage += damage;
      } else {
    ...
    this.bars.set(playerId, bar);
  }

  spawnBall(ball: Ball, damage: number, health: number): void {
    let bar = this.ballBars.get(ball);
    if (bar) {
      if (bar.delayTimer > 0) {
        bar.damage += damage;
      } else {
  ...
  predictDamage(playerId: string, damage: number): void {
    ...
    if (bar) {
      ...
      bar.delayTimer = DELAY_DURATION;
      bar.life = BAR_LIFETIME;
      bar.currentHealth = Math.max(0, bar.currentHealth - damage);
      bar.el.style.display = 'block';
      return;
    }
  ...
  update(dt: number, players: Map<string, { position: { x: number; y: number; z: number }; isDead: boolean }>, remotePlayers?: Map<string, { position: THREE.Vector3; isDead: boolean }>, balls?: Ball[]): void {
    ...
    for (const [playerId, bar] of this.bars) {
      ...
      // Use locally simulated position from RemotePlayer if available (follows knockback/pull)
