...
# Use client/src directory as project root
project_dir = Path('client/src')
...
# Non-functional extensions (documentation, config, etc.) - marked with brackets
NON_FUNCTIONAL_EXTENSIONS = {
    '.md',
    '.txt',
    '.json',
    '.xml',
...
def main():
    ...
    if args.ultra:
        ...
        max_size_width = max((len(format_size(f['size'], include_unit=False)) for f in file_results), default=8)
        max_lines_width = max((len(str(f['line_count'])) if f['line_count'] != '-' else 1 for f in file_results), default=4)
        max_ext_width = max((len(Path(f['path']).suffix.lower()) for f in file_results), default=3)
        max_index_width = len(str(len(file_results))) + 2  # +2 for brackets
        # Collect output lines
        output_lines = []
        # Print header with shortened labels to fit column widths
        index_label = '#'[:max_index_width - 2]
        size_label = 'KB'[:max_size_width]
        lines_label = 'Ln'[:max_lines_width]
        ext_label = 'Ext'[:max_ext_width]
        header = f"{index_label:>{max_index_width}} {size_label:>{max_size_width}} {lines_label:>{max_lines_width}} {ext_label:<{max_ext_width}} Path"
        print(header)
        output_lines.append(header)
        for idx, file in enumerate(file_results, 1):
            size_str = format_size(file['size'], include_unit=False)
            lines_str = str(file['line_count']) if file['line_count'] != '-' else '-'
            # Check if file is non-functional (documentation/config)
            ext = Path(file['path']).suffix.lower()
            is_non_functional = ext in NON_FUNCTIONAL_EXTENSIONS
            # Wrap index in brackets for non-functional files
            index_str = f"[{idx}]" if is_non_functional else str(idx)
            line = f"{index_str:>{max_index_width + 2}} {size_str:>{max_size_width}} {lines_str:>{max_lines_width}} {ext:<{max_ext_width}} {file['path']}"
            print(line)
        ...
        # Print file extension count summary
        ...
        # Wrap non-functional extensions in brackets
        ext_summary = ', '.join(
            f"{count} x [{ext}]" if ext in NON_FUNCTIONAL_EXTENSIONS else f"{count} x {ext}"
            for ext, count in sorted_exts
        )
        print(f"\n{ext_summary}")
        
        # Print simple stats
        script_files = [f for f in file_results if f['type'] == 'Script']
        non_script_files = [f for f in file_results if f['type'] == 'Asset']
        total_size = sum(f['size'] for f in file_results)
        script_size = sum(f['size'] for f in script_files)
        script_lines = sum(f['line_count'] for f in script_files if f['line_count'] != '-')
        
        print(f"\nTotal: {format_size(total_size)} {len(file_results)} files")
        print(f"Scripts: {format_size(script_size)} {len(script_files)} files {script_lines} lines")
        print(f"Assets: {len(non_script_files)} files")
        return
